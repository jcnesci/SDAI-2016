...
void draw() {
  ...
  if (!isFirstLoop) {
    ...
    float movieProgress = mT/mD;
    
    //if (millis() > (lastTime + 1000)) {
    //  lastTime = millis();
    //  println("t- " + mT + " | " + mD + " | "  + movieProgress);
    //}
    ...
    //if (movieProgress > 0.01) {
      ...
      beginShape();
      texture(glitchImage);
      vertex(-myMovie.width/2, -myMovie.height/2, 0, 0, 0);
      vertex(myMovie.width/2, -myMovie.height/2, 0, myMovie.width, 0);
      vertex(myMovie.width/2, myMovie.height/2, 0, myMovie.width, myMovie.height);
      vertex(-myMovie.width/2, myMovie.height/2, 0, 0, myMovie.height);
      endShape();
    //}
